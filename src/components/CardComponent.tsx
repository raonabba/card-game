    ...
    >
      ...
      <div className="w-full h-24 overflow-hidden bg-gray-700">
        {card.image ? (
          ...
          <div className="w-full h-full flex items-center justify-center text-4xl">{card.sign}</div>
        )}
      </div>
...
export function FieldCard({ minion, isSelected, isAttackable, isOwn, isAttacking, isHit, onClick }: FieldCardProps) {
  ...
  useEffect(() => {
    ...
    const y = isOwn ? -75 : 75;
    controls.start({ y, scale: 1.3, transition: { duration: 0.17, ease: 'easeIn' } })
      .then(() => controls.start({ y: 0, scale: 1, transition: { duration: 0.2, ease: 'easeOut' } }));
  }, [isAttacking, isOwn, controls]);

  useEffect(() => {
    if (!isHit) return;
    controls.start({ x: [-10, 10, -7, 7, -4, 4, 0], transition: { duration: 0.38 } });
  }, [isHit, controls]);
  ...
  return (
    ...
    >
      <div className="w-full h-16 bg-gray-700 overflow-hidden">
        {minion.image ? (
          <img src={minion.image} alt={minion.name} className="w-full h-full object-cover object-top"
            draggable={false} onError={(e) => { (e.target as HTMLImageElement).style.display = 'none'; }} />
        ) : (
          <div className="w-full h-full flex items-center justify-center text-2xl">{minion.sign}</div>
        )}
      </div>
      <div className="text-white text-[9px] text-center px-0.5 truncate font-bold">{minion.name}</div>
      <div className="mx-1 mt-0.5 h-1.5 bg-gray-600 rounded-full overflow-hidden">
        <div className={`h-full rounded-full transition-all duration-500 ${hpPct > 50 ? 'bg-green-400' : hpPct > 25 ? 'bg-yellow-400' : 'bg-red-400'}`}
